extends state
class_name enemy_float_investigate
...
func randomized_chase():
	chase_offset = Vector2(randf_range(-1, 1),randf_range(-1, 1))
...
func state_physics_update(delta: float):
	...
	for i in character_body_2d.get_slide_collision_count():
		...
		character_body_2d.velocity += collision.get_normal() * 5
